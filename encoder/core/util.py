...
def configure_backbone(backbone, size, pretrained_arg=False, num_classes_arg=7):
    return backbone(pretrained=pretrained_arg, rgb_stack_size=size,
                    num_classes=num_classes_arg)

def configure_backbone_forward_phase(backbone, pretrained_weights_path, size, pretrained_arg=True, num_classes_arg=7):
