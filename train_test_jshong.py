...
def main():
    ...
    dpath_train = os.path.join(dpath_root, 'train', '*.pkl')
    tpath_train = os.path.join(tpath_root,'train')
    dpath_val = os.path.join(dpath_root, 'val', '*.pkl')
    ...
    for epoch in range(1, args.num_epoch+1):
        ...
        if epoch % args.eval_freq == 0:
            f1_score = evaluation(model, val_loader, device, feature_dim)
            if f1_score > max_f1_score:
                max_f1_score = f1_score
                epoch_max = epoch
                torch.save(model.state_dict(), os.path.join(result_path, 'chckpoint_{:03d}.pt'.format(epoch)))

            str_print += ', f1_score: {:.4f} (max_f1_score: {:.4f} at epoch: {})'.format(f1_score, max_f1_score, epoch_max)
...
def evaluation(model, val_loader, device, feature_dim):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            correct += sum(scores == torch.argmax(y, dim=1))
            
            soft_total.extend(scores.cpu())
            # target_total.extend(y[:, 0].tolist())
            target_total.extend(torch.argmax(y, dim=1).cpu())
    ...
    # mAP = average_precision_score(target_total, soft_total, average='macro')
    
    accuracy = correct / count * 100
    
    macro_f1_score = f1_score(target_total, soft_total, average='macro')
    micro_f1_score = f1_score(target_total, soft_total, average='micro')
    weighted_f1_score = f1_score(target_total, soft_total, average='weighted')
    
    print("Accuracy: {}".format(accuracy))
                                 
    print("f1_score(label_lst, pred_lst, average = 'macro'): ", macro_f1_score)
    print("f1_score(label_lst, pred_lst, average = 'micro'): ", micro_f1_score)
    print("f1_score(label_lst, pred_lst, average = 'weighted'): ", weighted_f1_score)
